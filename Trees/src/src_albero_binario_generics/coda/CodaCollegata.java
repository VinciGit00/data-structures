...
/**
 * La classe <code>CodaCollegata</code> implementa il tipo di dato Coda mediante
 * una collezione di n record contenenti ciascuno una coppia (elem, next), dove
 * elem &egrave; un elemento della coda e prev &egrave; il puntatore al
 * successivo record della coda. La classe mantiene inoltre un puntatore
 * (inizio) al primo record della coda e un puntatore (fine) all'ultimo record
 * della coda.
 *
 */
public class CodaCollegata<D> implements Coda<D> {

	/**
	 * Puntatore al primo record della coda
	 */
	private Record<D> inizio = null;

	/**
	 * Puntatore all'ultimo record della coda
	 */
	private Record<D> fine = null;
	 ...
	 * 
	 * @return <code>true</code>, se la coda &egrave; vuota. <code>false</code>,
	 *         altrimenti
	 */
	 ...
	 */
	public void enqueue(D e) {
		if (this.isEmpty())
			this.inizio = this.fine = new Record<D>(e);
		else {
			this.fine.next = new Record<D>(e);
