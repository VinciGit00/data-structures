...
/**
 * Il tipo di dato Albero Binario, descritto dall'interfaccia
 * <code>AlberoBinario</code>, rappresenta alberi radicati in cui il grado di
 * ogni nodo e' al piu' 2. I figli di un albero binario hanno pertanto un ordine
 * e sono distinti in figlio sinistro e figlio destro. In un albero binario, un
 * nodo potrebbe avere un figlio sinistro, ma essere privo del figlio destro, e
 * viceversa.
 * 
 * Le operazioni supportate permettono di recuperare informazioni sull'albero
 * (radice, numero di nodi, grado, padre, figlio sinistro e figlio destro, e
 * contenuto informativo di un nodo), di modificarne il contenuto informativo e
 * la struttura (aggiungendo nodi oppure aggiungendo o rimuovendo interi
 * sottoalberi come figli sinistri o destri di nodi preesistenti), o di
 * visitarne i nodi in un ordine sistematico.
 * 
 * <p>
 * Per riferirci ai nodi di un albero binario useremo il tipo
 * <code>NodoBinario</code>. I riferimenti a nodi vengono generati dalle
 * operazioni che creano nodi (<code>aggiungiRadice</code>,
 * <code>aggiungiFiglioSin</code> e <code>aggiungiFiglioDes</code>) ed essi
 * possono essere in seguito utilizzati per riferirsi ai nodi dell'albero. <br>
 * Quando un sottoalbero viene staccato da un albero mediante l'operazione
 * <code>pota</code>, esso diventa una istanza di albero a s&eacute; stante, ma
 * i riferimenti ai nodi rimangono gli stessi. In modo simile, quando un albero
 * viene innestato in un altro albero, smette di essere un'istanza di albero a
 * s&eacute; stante, ma conserva gli stessi riferimenti ai suoi nodi.
 *
 */
public interface AlberoBinario<D extends Comparable<? super D>> {
	...
	/**
	 * Restituisce il numero di nodi interni dell'albero (un nodo e' interno se non
	 * e' foglia).
	 * 
	...
	/**
	 * Dice se l'albero e' perfettamente bilanciato o no. Un albero e' perfettament
	 * bilanciato se per ogni nodo, il numero di nodi del sottoalbero sinistro
	 * differisce al piu' di uno rispetto al numero di nodi del sottoalbero
	 * sinistro.
	 * 
	...
	/**
	 * Dice se l'albero e' completo o no. Un albero e' completo se tutte le foglie
	 * hanno profondita' h ed ogni altro nodo interno ha entrambi i figli.
	 * 
	...
	/**
	 * Restituisce una lista dove al primo posto si trova il minimo dell'albero e al
	 * secondo posto il massimo (impiega meno tempo che cercare il minimo e il
	 * massimo separatamente)
	 * 
	 * @return una lista con il minimo e il massimo (la lista puo' essere vuota se
	 *         l'albero e' vuoto)
	 */
	 ...
	 * @return il padre del nodo <code>v</code> nell'albero. <code>null</code>, se
	 *         <code>v</code> &egrave; la radice
	 */
	...
	// METODI ACCESSORI PER MODIFICARE IL CONTENUTO INFORMATIVO E LA STRUTTURA
	// DELL'ALBERO:
	/**
	 ...
	 * 
	 * @param <code>v1</code> e <code>v2</code> i nodi di cui si vuole scambiare il
	...
	/**
	 * Inserisce un sottoalbero nell'albero come figlio sinistro di un NodoBinario
	 * preesistente.
	 * 
	 * @param u      il NodoBinario cui aggiungere il sottoalbero
	 * @param albero l'albero la cui radice diventer&agrave; figlio sinistro di
	 *               <code>u</code>
	 */
	public void innestaSin(NodoBinario<D> u, AlberoBinario<D> albero);

	/**
	 * Inserisce un sottoalbero nell'albero come figlio destro di un NodoBinario
	 * preesistente.
	 * 
	 * @param u      il NodoBinario cui aggiungere il sottoalbero
	 * @param albero l'albero la cui radice diventer&agrave; figlio destro di
	 *               <code>u</code>
	 */
	public void innestaDes(NodoBinario<D> u, AlberoBinario<D> albero);

	/**
	 * Stacca e restituisce il sottoalbero sinistro di un certo NodoBinario
	 * dell'albero.
	 * 
	...
	/**
	 * Stacca e restituisce il sottoalbero destro di un certo NodoBinario
	 * dell'albero.
	 * 
	...
	/**
	 * Stacca e restituisce il sottoalbero radicato in un certo NodoBinario
	 * dell'albero.
	 * 
